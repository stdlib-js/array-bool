	...
	if ( nargs === 0 ) {
		...
		if ( isNonNegativeInteger( arg ) ) {
			...
			if ( HAS_ITERATOR_SYMBOL === false ) {
				throw new TypeError( format( '1uR29', arg ) );
			}
			if ( !isFunction( arg[ ITERATOR_SYMBOL ] ) ) {
				throw new TypeError( format( '1uR2A', arg ) );
			}
		...
		if ( !isArrayBuffer( buf ) ) {
			throw new TypeError( format( '1uR2B', buf ) );
		}
		byteOffset = arguments[ 1 ];
		if ( !isNonNegativeInteger( byteOffset ) ) {
			throw new TypeError( format( '1uR2C', byteOffset ) );
		}
		if ( nargs === 2 ) {
			...
			if ( !isNonNegativeInteger( len ) ) {
				throw new TypeError( format( '1uR2F', len ) );
			}
			if ( (len*BYTES_PER_ELEMENT) > (buf.byteLength-byteOffset) ) {
				throw new RangeError( format( '1uR2G', len*BYTES_PER_ELEMENT ) );
			}
...
setReadOnly( BooleanArray, 'from', function from( src ) {
	...
	var i;
	if ( !isFunction( this ) ) {
		throw new TypeError( format('1uR01') );
	}
	if ( !isBooleanArrayConstructor( this ) ) {
		throw new TypeError( format('1uRFj') );
	}
	nargs = arguments.length;
	if ( nargs > 1 ) {
		clbk = arguments[ 1 ];
		if ( !isFunction( clbk ) ) {
			throw new TypeError( format( '1uR2H', clbk ) );
		}
		if ( nargs > 2 ) {
			thisArg = arguments[ 2 ];
		}
	...
	if ( isObject( src ) && HAS_ITERATOR_SYMBOL && isFunction( src[ ITERATOR_SYMBOL ] ) ) { // eslint-disable-line max-len
		...
		if ( !isFunction( buf.next ) ) {
			throw new TypeError( format( '1uR2J', src ) );
		}
		if ( clbk ) {
...
setReadOnly( BooleanArray.prototype, 'at', function at( idx ) {
	...
	if ( !isInteger( idx ) ) {
		throw new TypeError( format( '1uR8A', idx ) );
	}
	len = this._length;
	buf = this._buffer;
	if ( idx < 0 ) {
...
setReadOnly( BooleanArray.prototype, 'every', function every( predicate, thisArg ) {
	...
	if ( !isFunction( predicate ) ) {
		throw new TypeError( format( '1uR3c', predicate ) );
	}
	buf = this._buffer;
...
setReadOnly( BooleanArray.prototype, 'fill', function fill( value, start, end ) {
	...
	if ( !isBoolean( value ) ) {
		throw new TypeError( format( '1uRFv', value ) );
	}
	buf = this._buffer;
	len = this._length;
	if ( arguments.length > 1 ) {
		if ( !isInteger( start ) ) {
			throw new TypeError( format( '1uR7f', start ) );
		}
		if ( start < 0 ) {
			start += len;
			if ( start < 0 ) {
		...
		if ( arguments.length > 2 ) {
			if ( !isInteger( end ) ) {
				throw new TypeError( format( '1uR2z', end ) );
			}
			if ( end < 0 ) {
				end += len;
				if ( end < 0 ) {
...
setReadOnly( BooleanArray.prototype, 'forEach', function forEach( fcn, thisArg ) {
	...
	if ( !isFunction( fcn ) ) {
		throw new TypeError( format( '1uR3c', fcn ) );
	}
	buf = this._buffer;
	for ( i = 0; i < this._length; i++ ) {
...
setReadOnly( BooleanArray.prototype, 'get', function get( idx ) {
	...
	if ( !isNonNegativeInteger( idx ) ) {
		throw new TypeError( format( '1uR2K', idx ) );
	}
	if ( idx >= this._length ) {
		return;
	}
...
setReadOnly( BooleanArray.prototype, 'includes', function includes( searchElement, fromIndex ) {
	...
	if ( !isBoolean( searchElement ) ) {
		throw new TypeError( format( '1uRFv', searchElement ) );
	}
	if ( arguments.length > 1 ) {
		if ( !isInteger( fromIndex ) ) {
			throw new TypeError( format( '1uR7f', fromIndex ) );
		}
		if ( fromIndex < 0 ) {
...
setReadOnly( BooleanArray.prototype, 'join', function join( separator ) {
	...
	if ( arguments.length > 0 ) {
		if ( !isString( separator ) ) {
			throw new TypeError( format( '1uR3F', separator ) );
		}
	} else {
		separator = ',';
	}
...
setReadOnly( BooleanArray.prototype, 'reduce', function reduce( reducer, initialValue ) {
	...
	if ( !isFunction( reducer ) ) {
		throw new TypeError( format( '1uR3c', reducer ) );
	}
	buf = this._buffer;
	len = this._length;
	if ( arguments.length > 1 ) {
...
setReadOnly( BooleanArray.prototype, 'set', function set( value ) {
	...
	if ( arguments.length > 1 ) {
		idx = arguments[ 1 ];
		if ( !isNonNegativeInteger( idx ) ) {
			throw new TypeError( format( '1uR2L', idx ) );
		}
	} else {
		idx = 0;
	}
	if ( isCollection( value ) ) {
		...
		if ( idx+N > this._length ) {
			throw new RangeError( format('1uR03') );
		}
	...
	if ( idx >= this._length ) {
		throw new RangeError( format( '1uR2M', idx ) );
	}
	buf[ idx ] = ( value ) ? 1 : 0;
});
...
setReadOnly( BooleanArray.prototype, 'slice', function slice( begin, end ) {
	...
	if ( arguments.length === 0 ) {
		...
		if ( !isInteger( begin ) ) {
			throw new TypeError( format( '1uR7e', begin ) );
		}
		if ( begin < 0 ) {
			begin += len;
			if ( begin < 0 ) {
		...
		if ( arguments.length === 1 ) {
			...
			if ( !isInteger( end ) ) {
				throw new TypeError( format( '1uR7f', end ) );
			}
			if ( end < 0 ) {
				end += len;
				if ( end < 0 ) {
...
setReadOnly( BooleanArray.prototype, 'sort', function sort( compareFcn ) {
	...
	if ( !isFunction( compareFcn ) ) {
		throw new TypeError( format( '1uR3c', compareFcn ) );
	}
	buf.sort( compare );
	return this;
...
setReadOnly( BooleanArray.prototype, 'toLocaleString', function toLocaleString( locales, options ) {
	...
	if ( arguments.length === 0 ) {
		...
		throw new TypeError( format( '1uRG5', locales ) );
	}
	if ( arguments.length < 2 ) {
		opts = {};
		...
		throw new TypeError( format( '1uR2V', options ) );
	}
	buf = this._buffer;
	out = [];
	for ( i = 0; i < this._length; i++ ) {
...
setReadOnly( BooleanArray.prototype, 'with', function copyWith( index, value ) {
	...
	if ( !isInteger( index ) ) {
		throw new TypeError( format( '1uR7e', index ) );
	}
	len = this._length;
	if ( index < 0 ) {
		index += len;
	...
	if ( index < 0 || index >= len ) {
		throw new RangeError( format( '1uRFP', index ) );
	}
	if ( !isBoolean( value ) ) {
		throw new TypeError( format( '1uR55', value ) );
	}
